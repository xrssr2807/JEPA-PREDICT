...
def train_epoch(model, dataloader, optimizer, criterion, device,
    ...
    ecg_iter = iter(ecg_loader) if distill_mode else None
...
def train_downstream(
    ...
    print(f"Phase 1: Linear Probe (frozen encoder{distill_tag})")
    print("=" * 60)
    ...
    for epoch in range(config.train.downstream_probe_epochs):
        ...
        test_loss, test_acc, auc, auc_list, prec, rec, f1, f05, report, _, _, _ = evaluate(
            model, test_loader, criterion, device, num_classes, is_dual=False,
        )
    ...
    for epoch in range(ft_epochs):
        train_loss, train_acc = train_epoch(
            model, train_loader, optimizer, criterion, device,
            scheduler=scheduler, sched_mode=sched_mode, is_dual=False, distill_mode=use_distill, ecg_encoder=ecg_encoder, proj_ppg=proj_ppg, proj_ecg=proj_ecg, ecg_loader=ecg_train_loader, distill_lambda=distill_lambda,
        )
        test_loss, test_acc, auc, auc_list, prec, rec, f1, f05, report, _, _, _ = evaluate(
            model, test_loader, criterion, device, num_classes, is_dual=False,
        )
